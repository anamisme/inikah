...
function getSecretKey() {
    if (!defined('INIKAH_SECRET')) {
        http_response_code(500);
        echo json_encode(['error' => 'Konfigurasi server tidak lengkap.']);
        exit;
    }
...
if (basename($_SERVER['SCRIPT_FILENAME']) === 'auth.php') {
    ...
    if (!defined('ADMIN_PASSWORD') || !defined('PETUGAS_PASSWORD')) {
        http_response_code(500);
        echo json_encode(['error' => 'Konfigurasi server tidak lengkap. Hubungi administrator.']);
        exit;
    }

    $VALID_PASSWORDS = [
        'admin'   => ADMIN_PASSWORD,
        'petugas' => PETUGAS_PASSWORD
    ];
